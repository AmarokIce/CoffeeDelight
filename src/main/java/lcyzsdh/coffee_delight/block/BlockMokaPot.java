...
public class BlockMokaPot extends BlockCoffeePotBase{
    public static final MapCodec<BlockMokaPot> CODEC = simpleCodec(BlockMokaPot::new);

    public BlockMokaPot(BlockBehaviour.Properties properties) {
        super(properties);
    }
        ...
        if(level.getBlockEntity(pos) instanceof CoffeePotEntity){
            ...
            if (!entity.isCanCook && player.getItemInHand(hand).is(ItemRegistry.COFFEE_FLOUR.get())) {
                ...
                if (itemInHand.getItem()==ItemRegistry.COFFEE_MAG.get())
                    itemInHand = entity.isFail ? new ItemStack(ItemRegistry.COFFEE_MAP.get("mag_fail_coffee").get()) : new ItemStack(ItemRegistry.COFFEE_MAP.get("mag_black_coffee").get());
                else if (itemInHand.is(ItemRegistry.COFFEE_CAP.get()))
                    itemInHand = entity.isFail ? new ItemStack(ItemRegistry.COFFEE_MAP.get("cap_fail_coffee").get()) : new ItemStack(ItemRegistry.COFFEE_MAP.get("cap_black_coffee").get());
