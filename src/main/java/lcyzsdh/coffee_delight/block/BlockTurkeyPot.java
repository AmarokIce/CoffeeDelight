...
public class BlockTurkeyPot extends BlockCoffeePotBase{
    public static final MapCodec<BlockTurkeyPot> CODEC = simpleCodec(BlockTurkeyPot::new);

    public BlockTurkeyPot(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    protected VoxelShape getShape(BlockState state, BlockGetter level, BlockPos pos, CollisionContext context) {
        return Block.box(2,0,3,14,9,14);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        if(level.getBlockEntity(pos) instanceof CoffeePotEntity) {
            ...
            if (!entity.isCanCook && player.getItemInHand(hand).is(ItemRegistry.COFFEE_FLOUR.get())) {
                ...
                String coffee=entity.hasSugar()? CoffeeType.TURKEY_COFFEE.getName():CoffeeType.ARABIC_COFFEE.getName();
                if (itemInHand.getItem()==ItemRegistry.COFFEE_MAG.get())
                    itemInHand = entity.isFail ? new ItemStack(ItemRegistry.COFFEE_MAP.get("mag_fail_coffee").get()) : new ItemStack(ItemRegistry.COFFEE_MAP.get("mag_"+coffee).get());
                else if (itemInHand.is(ItemRegistry.COFFEE_CAP.get()))
                    itemInHand = entity.isFail ? new ItemStack(ItemRegistry.COFFEE_MAP.get("cap_fail_coffee").get()) : new ItemStack(ItemRegistry.COFFEE_MAP.get("cap_"+coffee).get());
